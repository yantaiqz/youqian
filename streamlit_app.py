...
<style>
    ...
    header, [data-testid="stSidebar"], footer, .stDeployButton {
...
def render_static_navbar():
    # 修复：用三重引号+无缩进冲突的HTML，避免textwrap解析错误
    navbar_html = """
    <div style="
    ...
    """
    # 关键：unsafe_allow_html=True 必须加，否则HTML会被转义成文本
    st.markdown(navbar_html, unsafe_allow_html=True)

# -------------------------- 3. 业务逻辑与数据 (保持不变，修复小语法) --------------------------
TRANSLATIONS = {
...
def get_log_normal_percentile(value, median, shape_parameter):
    ...
    except Exception:  # 修复：通用异常捕获，避免程序崩溃
        return 0.0001
...
def render_metric_card(t, amount, currency, percentile, rank, color, lang_key):
    ...
    # 修复：HTML字符串用f-string时，引号转义+样式补全
    card_html = f"""
    <div style="
        background: white; 
